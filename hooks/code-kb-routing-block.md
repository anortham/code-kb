...
9. **Atomic Symbol Edits:** `replace_symbol_body(symbol_name, file_path, new_body, expected_body_hash)` verifies syntax through the extractor, checks concurrency hash, edits file, and updates SQLite index in one turn.
