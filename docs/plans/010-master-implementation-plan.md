...
- [ ] Initialize Cargo workspace in `c:\source\code-kb`:
...
- [ ] Implement Root & Workspace Discovery:
...
- [ ] Token-Minimized Output Serializers:
...
- [ ] Implement CLI subcommands mirroring every core tool:
...
- [ ] Add human-friendly terminal formatting (with `--json` flag for machine consumption).

### Phase 5: File Synchronization & Staleness Guards
- [ ] Tier 1: Synchronous Tool-Driven Sync
  - Trigger `julie-extract update --file <path>` immediately upon code mutations.
- [ ] Tier 2: Just-In-Time (JIT) Staleness Guard
...
- [ ] Tier 3: Background File Watcher
...
- [ ] Implement JSON-RPC stdio transport.
- [ ] Support both MCP 1.x handshake (`initialize` with `roots`) and MCP 2.0 (2026-07-28 stateless requests in `_meta`).
- [ ] Register the 7 Core Read Tools:
  - `codebase_outline`
  - `file_skeleton`
  - `find_symbol`
  - `get_symbol_body`
...
- [ ] Register the AST-Guided Edit Tool:
...
- [ ] Benchmark token consumption on real tasks (comparing bare agent vs `code-kb`).
- [ ] Verify Windows compatibility (handle management, path separators, WAL concurrency).
- [ ] Ship unified statically compiled executable.
