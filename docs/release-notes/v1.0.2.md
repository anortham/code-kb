...
`code-kb-core` 1.0.2 and `code-kb-cli` 1.0.2 are published. `cargo install code-kb-cli` builds `code-kb` only; `cargo binstall code-kb-cli` fetches the release archive below. Both need `julie-extract` 3.0.0 from the [julie-extractors releases](https://github.com/anortham/julie-extractors/releases) on `PATH` or in `JULIE_EXTRACT_BIN`.
