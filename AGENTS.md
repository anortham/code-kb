...
- Structural-fact categories: `CATEGORY_ALIASES` in `queries.rs` is the whole alias table
  ...
  A Flask route row names its literal endpoint, and an `add_url_rule` row names its view or says
  `no view function`.
  Facts and literals have separate limits, each
  with its own cap notice.
- Search ranking: `search_symbols` admits rows from four branches (exact name, FTS5 word match, trigram name substring, so `sha256` finds `parseSha256Sidecar`, and the members of a class whose whole name is a query word), then a deterministic Rust rerank in `queries.rs` credits each query term once from its strongest field (name whole token 3, name stem 2, the whole name of the member's class 3 when another word matches the member's name, so `Flask init` finds `Flask.__init__`, name substring 1, signature or docstring 1), weighted by the term's rarity across the index (a capped FTS5 match count per term), and adds the whole-name and all-words bonuses (the whole-name bonus is 100 for definition kinds and 60 for every other kind) and the kind, path, documentation, and test priors, and subtracts 2 for a function or method nested in a function, method, or constructor; `score` is that rerank score. After this first rerank, search considers at most the first three non-document code types among the top 20 results and reads up to 40 direct members of each in source order with the same filters. It admits or enriches only members whose own name or first 400 bytes of signature or docstring match a query term. The selected owner's name and first 400 bytes of documentation can then credit otherwise unmatched terms at 0.5 (`owner_context` in `--explain`) before a final rerank; this step does not recurse or change the exact-name bonuses. `lookup_symbol` lists exact names first, then names that start with the query, then names that contain it, and a `self.` attribute row after the other rows of its rank. A documentation link (Markdown and other document languages) is never an import row in lookup or search. In lookup and search text a member of a class or another code type shows as `Owner.name` (`ScriptInfo.__init__`), which the other tools accept as a name; JSON keeps the raw name. When some lookup rows only start with or contain the query, the header says how many are named exactly, and those other rows fold into one line of names, kinds, and counts; JSON keeps every row. A lookup that ends in `.` or `::` (`App.`) lists the members of the classes, modules, and namespaces with that name in source order. A Markdown heading with that name counts only when no code type has it. The `kind` filter of lookup and search accepts the stored kind or the kind the output shows (`attribute`, `section`, `link`, `element`, `table`). An attribute row whose signature starts with `self.`, `this.`, or `cls.` (except an F# member) shows the kind `attribute`, not `property`; JSON keeps the raw kind. An attribute row such as `self.debug = …` whose name a base class defines with a value or a property is a write, not a definition: lookup, search, and skeletons leave it out, and lookup still fills its limit. A base class named by a pending `extends` row counts only when no other class of that name is as close to the subclass's file. `blast_radius` shows a test method as `Class::test_name`. Rows from test files are hidden by default: a path rule in `queries.rs` (`is_test_path` and its SQL mirror `test_path_predicate`) hides the whole file, not only the symbols `julie-extract` flags, and `is_test: true` / `--include-tests` shows them again. A `lookup_symbol` row whose name equals the query is shown either way. When an exact lookup finds a definition, the import rows with the same name fold into one line with the count and the first three locations, and they do not count toward the limit. Equal scores break by name strength, the sum over query words of 3 for a whole-token name match, 2 for a stem match, 1 for a substring match, and 0 for none, then names that do not start with `_` before names that do; `--explain` reports the name strength as `name_strength`. `code-kb search --explain` prints the breakdown and the rerank timer; the MCP tool takes no `explain` parameter, and `--verbose` stays debug logging.
- Reference rules: a pending call whose receiver names an import binding in the caller's file matches
  ...
  `src/flask/app.py`). A relative import source (`../core/util.js`) names one file, resolved from
  the caller's folder with any script extension or an `index` file, and a receiver bound by such an
  ...
  as a test or fixture counts as a test to run only when it is a member of a test class or its
  name reads like one (`test_run`, `RoutesTest`); an app factory such as `create_app` is an
  impacted symbol.
...
- Language-agnostic callee filtering: `find_references(direction="callees")` and `get_symbol_context`
  ...
  A callee row names the definition the call reaches (``→ `AppContext.push` (src/flask/ctx.py:416)``),
  or up to three candidates, or says that more than three definitions have that name.
...
- `code-kb` pins the exact extractor version in `scripts/julie-pins.json` (currently `3.6.3`).
...
- Plugin distribution: every harness plugin (`.claude-plugin/plugin.json`, `.codex-plugin/plugin.json` with `.mcp.json`, root `plugin.json` with `mcp.json`, `mcp_config.json`, and `hooks.json` for Antigravity) starts the server and hooks through `bin/code-kb-launcher.cjs`. The launcher is a dependency-free Node script: on first run it downloads the release archive for the plugin's version from GitHub Releases into `~/.code-kb/dist/<version>/<target>/`, verifies the `.sha256` sidecar, unpacks it, and then executes `code-kb` with the given arguments. When that download fails, it runs the newest other cached version with a warning, so a version bump that reaches `main` before its release does not stop the server. `CODE_KB_BIN` names a binary to run instead; a binary or symlink at `~/.code-kb/bin/code-kb` overrides the download in every harness (Codex clones plugins into a cache and drops the environment of MCP servers, so only a file can reach it); and a plugin installed from a source checkout runs that checkout's `target/release/code-kb` when it exists. With one of these, `cargo build --release` plus a session restart is the whole dev loop. No binaries are committed to git. `tests/plugin/*.test.cjs` (run with `node --test`) cover the launcher and keep every manifest on the launcher and on one version.
- Release workflow: Documented step-by-step in `docs/RELEASING.md`; automated pre-flight check via `scripts/release-preflight.sh`, which also requires a fresh PASS from `scripts/release-corpus-check.py` (the previous release against the candidate on the real projects in `scripts/release-corpus.txt`) and a dogfood report from `scripts/release-dogfood.sh` with `## Result: PASS` and no `[open]` defect. The corpus check finds only changed output; the dogfood round finds wrong answers. Each release runs one dogfood round: a wrong answer is fixed before the tag with a regression test and a rerun of the failed call, not a new round; a gap (a true but incomplete answer, a heuristic miss, a feature request) does not block the release; only the user defers a wrong answer. A new julie-extract version is tagged after its code-kb dogfood round has no `[open]` defect. The launcher fetches by the version in `.claude-plugin/plugin.json`, so the version bump reaches `main` only in `git push --atomic origin main vX.Y.Z` with its tag, after CI passed on the commit before it; the release workflow waits for CI on the tagged commit.
