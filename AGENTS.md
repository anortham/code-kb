...
- Reference rules: a pending call whose receiver names an import binding in the caller's file matches
  ...
  `src/flask/app.py`). A relative import source (`../core/util.js`) names one file: its folders
  after `./` and `../` and its file name without a script extension end the target's path, and a
  receiver bound by such an import matches a class or namespace member only in that file. A bare pending call never matches a definition in another file when the
