...
3. **Symbol Definition:** `find_symbol(query="symbol_name", path="optional/subpath")` for exact/prefix lookup.
...
6. **Editing Prep / Call Graph:** `get_context_slice(symbol_name, file_path)` returns target body + immediate callee signatures + parameter types + tests in one call.
