...
#[test]
fn test_prune_orphaned_stores_lifecycle() {
    let temp = tempfile::tempdir().unwrap();
    let stores_dir = temp.path().join("stores");
    std::fs::create_dir_all(&stores_dir).unwrap();

    // 1. Create an active store pointing to an existing directory
    let live_repo = temp.path().join("live-repo");
    std::fs::create_dir_all(&live_repo).unwrap();

    let live_store = stores_dir.join("live-repo-hash123");
    std::fs::create_dir_all(&live_store).unwrap();
    let live_db = live_store.join("artifact.db");
    let conn = code_kb_core::open_read_write(&live_db).unwrap();
    conn.execute_batch(
    ...
    conn.execute(
        ...
        rusqlite::params![live_repo.to_str().unwrap()],
    )
    .unwrap();
    drop(conn);

    // 2. Create an orphaned store pointing to a non-existent directory (e.g. deleted worktree)
    let dead_repo = temp.path().join("deleted-worktree");
    let dead_store = stores_dir.join("deleted-worktree-hash456");
    std::fs::create_dir_all(&dead_store).unwrap();
    let dead_db = dead_store.join("artifact.db");
    let conn = code_kb_core::open_read_write(&dead_db).unwrap();
    conn.execute_batch(
        "CREATE TABLE artifact_metadata (key TEXT PRIMARY KEY, value TEXT NOT NULL);",
    )
    .unwrap();
    conn.execute(
        "INSERT INTO artifact_metadata (key, value) VALUES ('root_path', ?1);",
        rusqlite::params![dead_repo.to_str().unwrap()],
    )
    .unwrap();
    drop(conn);

    // 3. Dry-run prune should report the dead store without deleting it
    let would_prune = code_kb_core::prune_orphaned_stores_at(&stores_dir, true);
    assert_eq!(would_prune.len(), 1);
    assert_eq!(would_prune[0], dead_store);
    assert!(dead_store.exists(), "Dry run must not delete directory");
    assert!(live_store.exists(), "Live store must not be deleted");

    // 4. Real prune should delete the dead store
    let pruned = code_kb_core::prune_orphaned_stores_at(&stores_dir, false);
    assert_eq!(pruned.len(), 1);
    assert_eq!(pruned[0], dead_store);
    assert!(!dead_store.exists(), "Dead store must be deleted");
    assert!(live_store.exists(), "Live store must remain untouched");
}
